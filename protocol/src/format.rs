    ...
    fn from_chat(chat: &Chat, modifier: &Modifier) -> Self {
        ...
        let text_components: Vec<ComponentType> = match (&chat.translate, &chat.text, &chat.extra) {
            ...
            (Some(translate), Some(text), None) => {
                // Real Minecraft servers do occasionally send a chat
                // component that has BOTH a `translate` key and a `text`
                // key. The original code did `todo!()` here, which
                // panics the whole client the moment such a packet
                // arrives (e.g. certain 1.16 servers send translate+text
                // for some system messages). Render the text literally
                // and append the translated `with`-args after it so we
                // show *something* useful instead of crashing.
                log::warn!(
                    "format::from_chat: unimplemented (translate+text, no extra). \
                     translate={:?} text={:?} — falling back to literal text + with-args",
                    translate,
                    text
                );
                let mut components = Component::from_legacy_str(text, &modifier).list;
                for inner in &chat.with {
                    components.append(&mut Component::get_text(inner, &modifier).list);
                }
                components
            }
            (Some(translate), Some(text), Some(extra)) => {
                // Same family of unimplemented arm — translate + text +
                // extra. Concatenate text, then extra (which itself may
                // contain nested legacy strings), and ignore the
                // translate format string. We log so the user knows we
                // dropped the translate formatting.
                log::warn!(
                    "format::from_chat: unimplemented (translate+text+extra). \
                     translate={:?} text={:?} extra-len={} — falling back to text+extra",
                    translate,
                    text,
                    extra.len()
                );
                let mut components = Component::from_legacy_str(text, &modifier).list;
                components
                    .append(&mut Component::get_string_from_extra(extra, &modifier).list);
                components
            }
            (Some(translate), None, Some(extra)) => {
                // Note: the binding `text` here is actually the
                // `translate` value (the original code shadowed the
                // name). This arm fires when a server sends a chat
                // component with `translate` + `extra` but no top-level
                // `text`. Treat the translate string as a literal format
                // hint and append `extra` after it as a best-effort.
                log::warn!(
                    "format::from_chat: unimplemented (translate+extra, no text). \
                     translate={:?} extra-len={} — falling back to extra only",
                    translate,
                    extra.len()
                );
            ...
            (None, None, Some(extra)) => {
                // Chat component with ONLY `extra` (no top-level text,
                // no translate). The original code did `todo!()` here
                // but this is actually a perfectly legal packet shape
                // — many modern MC servers send `{extra: [...]}` as a
                // way to attach styled children to an empty parent.
                // Just render the extras.
                log::trace!(
                    "format::from_chat: chat with only `extra` (len={}) — rendering extras",
                    extra.len()
                );
                Component::get_string_from_extra(extra, &modifier).list
            }
