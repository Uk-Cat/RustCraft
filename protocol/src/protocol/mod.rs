...
impl Serializable for String {
    fn read_from<R: io::Read>(buf: &mut R) -> Result<String, Error> {
        ...
        // Cap the allocation at 1 MiB so a malformed packet can't
        // trigger a huge allocation that OOM-kills the client. Real MC
        // protocol strings (chat components, identifiers, etc.) are
        // always far smaller than this.
        if len < 0 || len > 1_048_576 {
            return Err(Error::Err(format!(
                "String length {} out of range (0..=1MiB)",
                len
            )));
        }
        let mut bytes = Vec::<u8>::with_capacity(len as usize);
        buf.take(len as u64).read_to_end(&mut bytes)?;
        // Lossy decode so a single bad byte doesn't crash the reader
        // thread. The replacement char (U+FFFD) is far better than
        // taking down the whole network connection.
...
impl Serializable for format::Component {
    fn read_from<R: io::Read>(buf: &mut R) -> Result<Self, Error> {
        ...
        buf.take(len as u64).read_to_end(&mut bytes)?;
        // Use lossy decoding so a malformed UTF-8 chat component from a
        // misbehaving server doesn't crash the client. The replacement
        // chars are better than a panic + disconnect.
        let ret = String::from_utf8(bytes)
            .unwrap_or_else(|e| String::from_utf8_lossy(&e.into_bytes()).into_owned());
...
impl Conn {
    ...
    pub fn read_packet(&mut self) -> Result<packet::Packet, Error> {
        ...
        if is_network_debug() {
            debug!(
                "about to parse id={:x}, dir={:?} state={:?} body_len={}",
                id,
                dir,
                self.state,
                buf.get_ref().len()
        ...
        let packet = packet::packet_by_id(self.protocol_version, self.state, dir, id, &mut buf)
            .map_err(|e| {
                // Annotate parse errors with the packet id + state so
                // the log shows exactly which packet failed to parse
                // and at what protocol version. Without this, the error
                // bubbles up as a bare "Failed to read packet" message
                // with no context.
                Error::Err(format!(
                    "packet_by_id failed: id=0x{:x} dir={:?} state={:?} protocol={}: {}",
                    id, dir, self.state, self.protocol_version, e
                ))
            })?;
