...
/// Sends a `UseEntity` packet with `ty = 1` (Attack) to the server, telling
/// it that the player has left-clicked (attacked) the entity identified by
/// `target_id`. The `sneaking` flag is only used on versions >= 1.16 where
/// the `UseEntity_Sneakflag` variant is in effect; older versions ignore it.
///
/// This mirrors vanilla Minecraft behaviour: when the crosshair is over an
/// entity and the player presses the attack button, an attack packet is sent
/// instead of (or in addition to) a block-dig packet.
pub fn send_use_entity_attack(
    conn: &mut Conn,
    target_id: i32,
    sneaking: bool,
) -> Result<(), Error> {
    let version = conn.get_version();
    if version < Version::V1_8 {
        // 1.7.x: UseEntity_Handsfree_i32, ty is a u8 (1 = attack)
        conn.write_packet(packet::play::serverbound::UseEntity_Handsfree_i32 {
            target_id,
            ty: 1,
        })
    } else if version < Version::V1_9 {
        // 1.8.x: UseEntity_Handsfree, ty is a VarInt (1 = attack)
        ...
        // 1.9 - 1.15.x: UseEntity_Hand variant (no sneaking flag yet)
        conn.write_packet(packet::play::serverbound::UseEntity_Hand {
            ...
            target_y: 0.0,
            target_z: 0.0,
            hand: VarInt(Hand::MainHand.ordinal()),
        })
    } else {
        // 1.16+: UseEntity_Sneakflag includes the sneaking flag
