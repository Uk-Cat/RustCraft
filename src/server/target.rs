...
/// Casts a ray from `s` in direction `d` (normalised) and finds the closest
/// entity whose `Bounds` (entity hitbox) intersects the ray within `max`
/// distance. The local player (identified by `exclude`) is skipped so the
/// player never attacks themselves.
///
/// Unlike `trace_ray` (which is a voxel-grid traversal used for blocks),
/// this iterates the entire `entity_map` and runs a ray-vs-AABB intersection
/// against each entity's world-space hitbox. The world-space hitbox is
/// computed by translating the entity's `Bounds` AABB by its current
/// `Position`.
///
/// On success, returns the server-side entity id (`i32`) of the closest hit
/// entity. This is the value that needs to be sent in the `UseEntity` packet
/// to attack the entity.
pub fn trace_ray_entities<H>(
    world: &bevy_ecs::world::World,
    entity_map: &HashMap<i32, Entity, H>,
    exclude: Entity,
    max: f64,
    s: cgmath::Vector3<f64>,
    d: cgmath::Vector3<f64>,
...
{
    use crate::entity::{Bounds, EntityType, Position as EntityPosition};
    use cgmath::MetricSpace;

    let mut best: Option<(i32, f64)> = None;

    for (server_id, ecs_entity) in entity_map.iter() {
        if *ecs_entity == exclude {
            continue;
        }
        // Skip entities that don't have a Position + Bounds (e.g. unmodelled
        // entity types that haven't had a hitbox assigned yet).
        let entity_ref = world.entity(*ecs_entity);
        let pos = match entity_ref.get::<EntityPosition>() {
            Some(p) => p,
            None => continue,
        };
        let bounds = match entity_ref.get::<Bounds>() {
            Some(b) => b,
            None => continue,
        };

        // Skip non-combat entities (dropped items, XP orbs, projectiles,
        // paintings, item frames, weather, etc.). When the crosshair is over
        // one of these, the player's left-click should fall through to block
        // breaking rather than being swallowed by an entity that can't be
        // damaged anyway.
        if let Some(entity_type) = entity_ref.get::<EntityType>() {
            if !entity_type.is_attackable() {
                continue;
            }
        }

        // Translate the entity-local AABB into world space.
        let world_bound = bounds.bounds.add_v(pos.position);
        if let Some(hit) = intersects_line(world_bound, s, d) {
            let dist = hit.distance2(s).sqrt();
            if dist <= max {
                match best {
                    Some((_, best_dist)) if dist >= best_dist => {}
                    _ => best = Some((*server_id, dist)),
                }
            }
        }
    }
