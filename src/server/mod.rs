...
use leafish_protocol::protocol::packet::{
    send_client_status, send_drop_item, send_use_entity_attack, ClientStatus, Hand,
};
...
impl Server {
    pub fn connect(
        ...
        info!(
            "Connecting to server {} (protocol v{}, fml={:?})",
            address, protocol_version, fml_network_version
        );
        let mut conn = protocol::Conn::new(address, protocol_version).map_err(|e| {
            error!("Failed to open TCP connection to {}: {}", address, e);
            e
        })?;
        ...
        let port = conn.port;
        debug!(
            "Sending Handshake to {}:{} (host field={:?})",
            conn.host, conn.port, host
        );
        ...
        debug!("LoginStart sent for account {}", account.name);
        ...
        loop {
            match conn.read_packet()? {
                protocol::packet::Packet::SetInitialCompression(val) => {
                    debug!("Login: SetInitialCompression threshold={}", val.threshold.0);
                    conn.set_compression(val.threshold.0);
                }
                protocol::packet::Packet::EncryptionRequest(val) => {
                    debug!("Login: EncryptionRequest server_id={:?}", val.server_id);
                ...
                protocol::packet::Packet::EncryptionRequest_i16(val) => {
                    debug!("Login: EncryptionRequest_i16 server_id={:?}", val.server_id);
                    server_id = Rc::new(val.server_id);
                    public_key = Rc::new(val.public_key.data);
                    verify_token = Rc::new(val.verify_token.data);
                    break;
                ...
                protocol::packet::Packet::LoginSuccess_String(val) => {
                    ...
                    info!("Login success (offline): username={} uuid={}", val.username, val.uuid);
                ...
                protocol::packet::Packet::LoginDisconnect(val) => {
                    error!("Login rejected by server: {:?}", val.reason);
                    return Err(protocol::Error::Disconnect(val.reason))
                }
                val => {
                    error!("Unexpected packet during login phase 1: {:?}", val);
                    return Err(protocol::Error::Err(format!("Wrong packet 1: {:?}", val)));
                }
            };
        }
        ...
        debug!("Login: joining Mojang session server (server_id={:?})", server_id);
        account.join_server(&server_id, &shared, &public_key).map_err(|e| {
            error!("Session server join failed for {}: {}", account.name, e);
            e
        })?;
        debug!("Login: session join OK, sending EncryptionResponse");
        ...
        debug!("Login: encryption enabled, awaiting LoginSuccess");
        ...
        loop {
            match conn.read_packet()? {
                ...
                protocol::packet::Packet::LoginSuccess_String(val) => {
                    info!("Login success: username={} uuid={}", val.username, val.uuid);
                ...
                protocol::packet::Packet::LoginDisconnect(val) => {
                    error!("Login rejected by server (post-encryption): {:?}", val.reason);
                    return Err(protocol::Error::Disconnect(val.reason))
                }
    ...
    fn connect0(
        ...
        info!(
            "Login complete, entering Play state (uuid={:?}, protocol v{}, forge mods={})",
            uuid,
            protocol_version,
            forge_mods.len()
        );
        ...
        info!("connect0: Server::new returned, wiring up hud_context");
        server.hud_context.write().server = Some(server.clone());

        inner_server.replace(server.clone());
        info!("connect0: sending initial render-list-computer tick");
        render_list_computer.0.send(true).unwrap();
        info!("connect0: returning server from connect0");
        server
    }

    #[allow(unused_must_use)]
    fn spawn_reader(mut read: protocol::Conn, server: Arc<Mutex<Option<Arc<Server>>>>) {
        // Spawn the reader thread with an explicit 8 MB stack.
        //
        // The Windows default thread stack is only 1 MB (vs 8 MB on
        // Linux). The NBT parser used by the JoinGame / chunk-data
        // packet handlers is recursive, and a 30 KB JoinGame
        // dimension_codec NBT can have enough nesting to overflow a
        // 1 MB stack. A stack overflow on Windows kills the process
        // immediately — no Rust panic, no catch_unwind, no log output,
        // no crash file. The process just vanishes.
        //
        // Setting an 8 MB stack matches the Linux default and gives
        // the NBT parser plenty of headroom. This is belt-and-suspenders
        // alongside the MAX_NBT_DEPTH=512 recursion cap in the NBT
        // parser itself.
        ...
        thread::Builder::new()
            ...
            .spawn(move || {
            info!("Network reader thread started");
            // Build the rayon pool with an explicit 8 MB stack per
            // worker. The default rayon worker stack size on Windows
            // is only 1 MB, and the chunk-data parsing dispatched to
            // this pool involves NBT decoding (recursive) and chunk
            // section construction (deep call chains). A 1 MB stack
            // can silently overflow on a complex chunk, killing the
            // entire process.
            ...
            loop {
                ...
                // Wrap read_packet() itself in catch_unwind so that a
                // panic INSIDE the parser (packet_by_id / NBT / String
                // readers) is caught and logged with full context.
                //
                // Previously catch_unwind only wrapped pck.map() and the
                // handler dispatch — but read_packet() is where the
                // actual parsing happens, and it was OUTSIDE the catch.
                // That meant a parser panic killed the reader thread
                // silently (the panic hook SHOULD have logged it, but if
                // anything went wrong with the hook — file lock
                // contention, hook not yet installed, etc. — we'd lose
                // all evidence). This wraps the parser too so we get a
                // guaranteed "PARSER PANIC" log line on any parser
                // panic.
                let read_result = panic::catch_unwind(AssertUnwindSafe(|| {
                    read.read_packet()
                }));
                info!(
                    "reader: read_packet returned (packet #{}), result is {}",
                    packet_count + 1,
                    match &read_result {
                        Ok(Ok(_)) => "Ok",
                        Ok(Err(_)) => "Err",
                ...
                let pck = match read_result {
                    Ok(Ok(pck)) => pck,
                    Ok(Err(err)) => {
                        // Normal read error (connection closed, etc.).
                        // Log and break out of the loop.
                        error!(
                            "Error reading packet from server (after {} packets): {}",
                            packet_count, err
                        );
                        ...
                        warn!("Network reader thread exiting due to read error");
                        break;
                    }
                    Err(panic_payload) => {
                        // Parser panicked. The panic hook has already
                        // written the backtrace to the log; here we add
                        // a high-level marker that ties the panic to
                        // this packet #, so the user sees a clear
                        // "(parser panic on packet #N)" line.
                        let msg: String = if let Some(s) =
                        ...
                        error!(
                            "PARSER PANIC on packet #{}: {}",
                            packet_count + 1,
                            msg
                        );
                        // Don't break — try to keep reading. If the
                        // connection is genuinely broken, the next
                        // read_packet() will return Err and we exit
                ...
                {
                    // pck is now a successfully-parsed packet.
                    packet_count += 1;
                    // Always log which packet variant we received and
                    // its sequence number, even when network_debug is
                    // off. This is critical for diagnosing "joined
                    // then crashed" scenarios — without this line,
                    // we can't tell which packet handler ran last.
                    // Use a truncated Debug representation so chunk
                    // data doesn't blow up the log.
                    let pck_debug = format!("{:?}", pck);
                    let pck_summary = if pck_debug.len() > 200 {
                        // Find the first '(' which separates the variant
                        // name from its payload, and log just the name.
                        match pck_debug.find('(') {
                            Some(idx) => pck_debug[..idx].to_string(),
                            None => pck_debug[..200].to_string(),
                        }
                    } else {
                        pck_debug
                    };
                    ...
                    // Log right before AND right after the handler
                    // dispatch so we can tell whether the process died
                    // DURING the dispatch (no "dispatch complete" line)
                    // vs. AFTER it (no "read_packet returned" line for
                    // packet #N+1). This is critical: catch_unwind
                    // catches Rust panics but NOT Windows SEH exceptions
                    // (e.g. stack overflow), so a silent death during
                    // the dispatch would otherwise leave no trace.
                    info!(
                        ...
                        // ANY packet's parser or handler is logged with
                        // full context instead of silently killing the
                        // reader thread. Without this, a single
                        // unimplemented!()/todo!()/unwrap() panic on a
                        // malformed packet takes down the whole network
                        // reader and the log just stops mid-parse with
                        // no clue what went wrong.
                        //
                        // AssertUnwindSafe is acceptable here because:
                        //   - the only shared mutable state touched is
                        //     `server` (Arc<Server>) which uses internal
                        //     locking for all its fields
                        //   - `threads` (rayon pool) is also internally
                        //     synchronized
                        //   - `packet_count` is a local u64 we don't
                        //     touch inside the closure
                        let dispatch_result = panic::catch_unwind(AssertUnwindSafe(|| {
                            ...
                            MappedPacket::JoinGame(join) => {
                                ...
                                info!(
                                    "JoinGame handler: gamemode={}, entity_id={}, dimension_id={:?}, dimension_name={:?}, world_name={:?}",
                                    gamemode, entity_id, dimension_id, dimension_name, world_name
                                );

                                info!("JoinGame handler: calling on_game_join");
                                server.on_game_join(gamemode, entity_id);
                                info!("JoinGame handler: on_game_join returned");
                                ...
                                info!("JoinGame handler: dimension resolved = {:?}", dimension);

                                if let Some(dimension) = dimension {
                                    info!("JoinGame handler: calling set_dimension");
                                    server.world.set_dimension(dimension);
                                    info!("JoinGame handler: set_dimension returned");
                                }
                                info!("JoinGame handler: done");
                            }
                            ...
                            _ => {
                                // Packet variant has no dedicated handler —
                                // log it so we can see what's being
                                // silently dropped. Truncate to keep the
                                // log readable.
                                let dbg = format!("{:?}", pck);
                                let summary = if dbg.len() > 200 {
                                    match dbg.find('(') {
                                        Some(idx) => dbg[..idx].to_string(),
                                        None => dbg[..200].to_string(),
                                    }
                                } else {
                                    dbg
                                };
                                debug!("unhandled packet: {}", summary);
                            }
                        }
                        }));
                        ...
                        match dispatch_result {
                            Ok(()) => {
                                // Handler ran cleanly.
                                info!(
                                    "reader: dispatch complete for packet #{} ({})",
                                    packet_count, pck_summary
                                );
                            }
                            Err(panic_payload) => {
                                // A packet parser or handler panicked.
                                // The panic hook has already written the
                                // backtrace to the log file; here we add
                                // a high-level marker that ties the
                                // panic to this packet #, so the user
                                // sees a clear "(packet #N handler
                                // panicked)" line in the log.
                                let msg: String = if let Some(s) = panic_payload
                                ...
                                error!(
                                    "HANDLER PANIC on recv packet #{} ({}): {}",
                                    packet_count, pck_summary, msg
                                );
                                // Don't break the loop — the panic hook
                                // already captured the backtrace, and the
                                // next packet may well be fine (the
                                // panicking handler may have touched
                                // per-packet state only). If the
                                // connection is genuinely broken, the
                                // next read_packet() will return Err and
                                // we'll exit through the normal error
                                // path below.
                            }
                        }
                } // close inner block (pck scope)
            } // close loop
            warn!(
                "Network reader thread exited after receiving {} packets total",
                packet_count
            );
        })
    ...
    fn spawn_light_updater(_server: Arc<Mutex<Option<Arc<Server>>>>) -> Sender<LightUpdate> {
        let (tx, rx) = unbounded();
        // Explicit 8 MB stack — same rationale as the reader thread. The
        // light updater body is currently a no-op (the actual work is
        // commented out), but if it is ever re-enabled it will do
        // recursive flood-fill lighting that needs headroom.
        const LIGHT_STACK_SIZE: usize = 8 * 1024 * 1024;
        thread::Builder::new()
            .name("light-updater".to_string())
            .stack_size(LIGHT_STACK_SIZE)
            .spawn(move || loop {
            ...
            .expect("failed to spawn light-updater thread");
        tx
    }
    ...
    fn spawn_render_list_computer(
        ...
        // Explicit 8 MB stack — same rationale as the reader thread.
        // compute_render_list -> do_render_queue is recursive, with the
        // depth bounded by the BFS expansion of visible chunk sections.
        // On Windows the default 1 MB stack can be exhausted by a
        // pathological view (e.g., 32 chunk render distance across an
        // open plain), causing a silent stack-overflow process abort.
        const RLC_STACK_SIZE: usize = 8 * 1024 * 1024;
        thread::Builder::new()
            .name("render-list-computer".to_string())
            .stack_size(RLC_STACK_SIZE)
            .spawn(move || loop {
            ...
            .expect("failed to spawn render-list-computer thread");
        (tx, erx)
    }
    ...
    fn new(
        ...
        info!("Server::new: InventoryContext created");
    ...
    pub fn on_left_click(&self, focused: bool, shift: bool) {
        if focused {
            ...
            if let Some(player) = self.player.load().as_ref() {
                let player_entity = player.1;

                // Read the player's gamemode so we can pick the correct attack
                // reach. Vanilla Minecraft lets Creative-mode players reach
                // further than Survival players when interacting with entities.
                // Here we use 3.0 blocks for Survival (and other non-creative
                // modes) and 5.0 blocks for Creative, as configured.
                let gamemode = *self
                    .entities
                    .read()
                    .world
                ...
                let reach = match gamemode {
                    GameMode::Creative => 5.0,
                    // Survival, Adventure, Spectator, NotSet all use the
                    // shorter reach. (Spectator can't actually damage
                    // entities, but the server enforces that — we still
                    // raycast the shorter distance to match vanilla feel.)
                    _ => 3.0,
                };

                // Always send the arm-swing animation so other players see the
                // swing regardless of whether we hit an entity or a block.
                packet::send_arm_swing(self.conn.write().as_mut().unwrap(), Hand::MainHand)
                    .unwrap();

                // Try to attack an entity first. If an entity is hit, we
                // send a `UseEntity` attack packet and deliberately do NOT
                // set `mouse_buttons.left = true`, so the `apply_digging`
                // ECS system does not also start breaking the block behind
                // the entity.
                let hit_entity = {
                    let entities_read = self.entities.read();
                    let entity_map = self.entity_map.read();
                    target::trace_ray_entities(
                        &entities_read.world,
                        &entity_map,
                        player_entity,
                        reach,
                ...
                // No entity in the crosshair: fall back to the original
                // block-breaking behaviour by arming `mouse_buttons.left`.
                let mut entities = self.entities.write();
                let mut player = entities.world.entity_mut(player_entity);
                let mut mouse_buttons = player.get_mut::<MouseButtons>().unwrap();
    ...
    fn on_game_join(&self, gamemode: u8, entity_id: i32) {
        info!("on_game_join: start (gamemode={}, entity_id={})", gamemode, entity_id);
        let gamemode = GameMode::from_int((gamemode & 0x7) as i32);
        info!("on_game_join: creating local player entity");
        let player = entity::player::create_local(&mut self.entities.clone().write());
        info!("on_game_join: player entity created (id={:?})", player);
        if let Some(info) = self.players.read().get(&self.uuid) {
            info!("on_game_join: found player info, setting skin");
            let mut entities = self.entities.write();
            ...
            info!("on_game_join: skin set");
        } else {
            info!("on_game_join: no player info yet (players map empty), skipping skin");
        }
        info!("on_game_join: updating hud game mode");
        self.hud_context.write().update_game_mode(gamemode);
        info!("on_game_join: setting player GameMode component");
        *self
            .entities
            .write()
            .world
            ...
            .get_mut::<GameMode>()
            .unwrap() = gamemode;
        info!("on_game_join: setting PlayerMovement.flying");
        self.entities
            .write()
            .world
            .entity_mut(player)
        ...
        info!("on_game_join: inserting into entity_map");
        self.entity_map.write().insert(entity_id, player);
        info!("on_game_join: storing player in ArcSwap");
        self.player.store(Some(Arc::new((entity_id, player))));

        info!("on_game_join: sending ClientSettings");
        ...
        info!("on_game_join: ClientSettings sent, sending brand plugin message");
        ...
        info!("on_game_join: brand sent, done");
        /*packet::send_position_look(
            self.conn.write().as_mut().unwrap(),
