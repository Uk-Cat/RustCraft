...
impl Game {
    ...
    pub fn connect_to(
        ...
        // Spawn the connect thread with an explicit 16 MB stack.
        //
        // The Windows default thread stack is only 1 MB. Server::connect
        // -> connect0 -> Server::new runs on THIS thread, and the
        // initialization chain (World::new, InventoryContext::new,
        // entity::add_systems, etc.) is deep enough that on Windows we
        // were silently stack-overflowing during Server::new while the
        // network reader thread was simultaneously parsing the JoinGame
        // packet. A stack overflow on Windows triggers
        // STATUS_STACK_OVERFLOW — the process is killed immediately,
        // bypassing Rust's panic hook, so NO log output, NO crash file,
        // NO "PARSER PANIC" line. The user just sees the window close.
        //
        // 16 MB matches the Linux default (8 MB) with extra headroom.
        const CONNECT_STACK_SIZE: usize = 16 * 1024 * 1024;
        info!("Spawning connect thread (stack={} bytes)", CONNECT_STACK_SIZE);
        let result = thread::Builder::new()
            .name("server-connect".to_string())
            .stack_size(CONNECT_STACK_SIZE)
            .spawn(move || {
            ...
            .expect("failed to spawn server-connect thread")
            .join();
        info!("Connect thread joined");
        match result {
            Ok(result) => {
                match result {
                    Ok(srv) => {
                        info!("connect() Ok, storing server");
                        self.server.store(Some(srv));
                        info!("connect() done, returning Ok");
                        Ok(())
                    }
                    Err(err) => {
                        let str = err.to_string();
                        error!("connect() returned Err: {}", str);
            ...
            Err(join_err) => {
                // The connect thread panicked. The panic hook has
                // already written the panic + backtrace to the log
                // file; surface a pointer to that file in the UI error
                // so the user knows where to look.
                let log_hint = console::Console::log_file_path()
                    .map(|p| format!(" (see log: {})", p.display()))
                    .unwrap_or_default();
                error!(
                    "Server connect thread panicked: {:?}{}",
                    join_err, log_hint
                );
                Err(Error::Err(format!(
                    "Internal error while connecting{}",
                    log_hint
                )))
            }
        }
...
struct Opt {
    ...
    /// Override the path the .log file is written to. By default the
    /// log lives at <cache_dir>/leafish/client.log. Pass this flag (or
    /// set LEAFISH_LOG_FILE) to put it somewhere easier to find, e.g.
    /// `--log-file ./client.log` next to the binary.
...
fn main() {
    ...
    // Pick the log file path: explicit CLI flag > env var > default
    // cache dir. Resolving this before constructing the Console means
    // the panic hook is wired up before *anything* else runs, so even
    // an early init crash is captured to disk.
    let log_file_path: Option<std::path::PathBuf> = opt
        .log_file
        .as_ref()
        .map(std::path::PathBuf::from)
        .or_else(|| std::env::var("LEAFISH_LOG_FILE").ok().map(std::path::PathBuf::from));

    #[allow(clippy::arc_with_non_send_sync)]
    let con = Arc::new(Mutex::new(console::Console::with_log_file(log_file_path)));
    ...
    if let Some(path) = console::Console::log_file_path() {
        info!("Leafish log file: {}", path.display());
    } else {
        info!("Leafish log file: <not initialised>");
    }
    // Stamp the build version into the log on startup so we can ALWAYS
    // tell which exact binary the user ran. This is critical for
    // diagnosing "stale .zip" reports where the user thinks they're on
    // the latest build but actually have an older .exe. The git hash
    // is captured at build time by build.rs and baked into the binary
    // via env!().
    info!(
        "RustCraft build: {} (built {})",
        env!("LEAFISH_BUILD_GIT_HASH"),
        env!("LEAFISH_BUILD_TIME")
    );
    ...
    // Configure log levels + auto-enable network packet logging.
    //
    // IMPORTANT: we MUST release the Console mutex before any `info!` /
    // `debug!` / `trace!` call, because the global `ConsoleProxy` logger
    // tries to acquire that same mutex to write the log line. parking_lot
    // is non-reentrant, so logging while holding the lock deadlocks the
    // calling thread. (`configure_and_report` deliberately does NOT log
    // from inside `configure` — it just returns the network-debug status
    // so we can log it here, after the guard drops.)
    let network_debug_enabled = { con.lock().configure_and_report(&settings) };
    if network_debug_enabled {
        info!("Network packet logging is enabled");
    } else {
        info!(
            "Network packet logging is disabled (file log level is below Debug)"
        );
    }
